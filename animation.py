...
class KeyFrames:
    ...
    def value(self, time):
        ...
        return self.interpolate(self.values[time_index - 1], self.values[time_index], (time - self.times[time_index - 1]) / (self.times[time_index] - self.times[time_index - 1]))

class TransformKeyFrames:
    """ KeyFrames-like object dedicated to 3D transforms """
    def __init__(self, translate_keys, rotate_keys, scale_keys):
        """ stores 3 keyframe sets for translation, rotation, scale """
        self.translate = translate_keys
        self.rotate = rotate_keys
        self.scale = scale_keys

    def value(self, time):
        """ Compute each component's interpolation and compose TRS matrix """
        t = translate(KeyFrames(self.translate).value(time))
        r = quaternion_matrix(KeyFrames(self.rotate, quaternion_slerp).value(time))
        s = scale(KeyFrames(self.scale).value(time))
        return t@r@s
